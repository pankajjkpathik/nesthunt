...
export function PromiseLedgerTab({ placeId }: { placeId?: string }) {
  const { data = [], isLoading } = usePlacePromises(placeId);
  const { data: evidence = [] } = usePlaceEvidence(placeId);
    ...
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          Save the place first to track promises.
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Announced projects and government commitments, with delivery status.
          </p>
        </div>
        <Button
          size="sm"
          onClick={async () => {
            try {
              await create.mutateAsync({ place_id: placeId, promise: "New promise" });
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {data.map((row) => (
            <PromiseRow key={row.id} row={row} evidence={evidence} />
          ))}
        </div>
      )}
    </div>
...
function PromiseRow({
  ...
  const remove = useDeletePromise(row.place_id);
