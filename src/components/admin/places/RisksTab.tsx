...
export function RisksTab({ placeId }: { placeId?: string }) {
  const { data = [], isLoading } = usePlaceRisks(placeId);
    ...
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          Save the place first to add risks.
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={async () => {
            try {
              await create.mutateAsync({ place_id: placeId, title: "New risk" });
      ...
      {isLoading ? (
        ...
        <>
          <div className="space-y-3">
            {paged.map((row) => (
              <RiskRow key={row.id} row={row} evidence={evidence} />
            ))}
          </div>
          {pageCount > 1 ? (
...
function RiskRow({ row, evidence }: { row: PlaceRiskRow; evidence: { id: string; title: string }[] }) {
  const [draft, setDraft] = useState<PlaceRiskRow>(row);
  const [dirty, setDirty] = useState(false);
  const update = useUpdateRisk();
  const remove = useDeleteRisk(row.place_id);
