...
export function EvidenceTab({ placeId }: Props) {
  const { data = [], isLoading } = usePlaceEvidence(placeId);
    ...
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          Save the place first to attach research evidence.
        </CardContent>
      </Card>
    );
  }
  ...
  async function addEmpty() {
    ...
    setCreating(true);
    try {
      await create.mutateAsync({ place_id: placeId, title: "New evidence" });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Sources, citations, documents and photos that back this place's intelligence.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            No evidence records yet. Every published Place needs at least one.
          </CardContent>
        </Card>
      ) : (
